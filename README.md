...
|API9:2023 - Improper Inventory Management|APIs tend to expose more endpoints than traditional web applications, making proper and updated documentation highly important. A proper inventory of hosts and deployed API versions also are important to mitigate issues such as deprecated API versions and exposed debug endpoints.|[ ]|The vulnerability is relatively easy to encounter if the API has documentation that has not been updated and might not cover every critical detail, like every API endpoint or there's a microservice that handles user validation, which can be disabled by accident. Another way of exposing the API is to mismanage the different versions and accidentally creating a backdoor through older or development version of the API, which might not have the same security measures as the production ones.|
|API10:2023 - Unsafe Consumption of APIs|Developers tend to trust data received from third-party APIs more than user input, and so tend to adopt weaker security standards. In order to compromise APIs, attackers go after integrated third-party services instead of trying to compromise the target API directly.|[ ]|Developers are more often than not naive towards third-party API security. Usually developers trust APIs that their API utilizes instead of implementing "don't trust client" mentality more broadly, like "don't trust outsiders". I'm not sure, but this vulnerability could also target libraries (more of an issue with npm packages). When incoming data is sanitiezed correctly the API should be save from attacks even from third-party APIs.|
